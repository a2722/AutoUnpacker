# -*- coding: utf-8 -*-
"""共享密码本对话框（UI 层）：长期密码编辑 + 临时密码查看，所有监听目录共用。
...
关键入口：PasswordBookDialog
依赖：PyQt5、state.AppState、style（PALETTE）、passwords.book（行文本解析 / 格式化）
...
from ..passwords.book import (format_share_code_text, parse_password_text,
